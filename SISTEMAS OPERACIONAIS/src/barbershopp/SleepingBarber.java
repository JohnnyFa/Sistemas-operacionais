...
class Customer extends Thread {
  ...
  public void run() {   
    while (notCut) {  // as long as the customer is not cut 
      ...
      if (numberOfFreeSeats > 0) {  //if there are any free seats
        System.out.println("Cliente " + this.iD + " esta na sala de espera.");
	...
	barber.acquire();  // now it's this customers turn but we have to wait if the barber is busy
        ...
        System.out.println("Nao ha cadeiras livres. Cliente " + this.iD + " deixou a barbearia.");
  ...
  public void get_haircut(){
    System.out.println("Cliente " + this.iD + " tendo cabelo cortado");
    try {
    sleep(100);
    System.out.println("cliente" + this.iD + "deixou a barbearia");
    } catch (InterruptedException ex) {}
  }
